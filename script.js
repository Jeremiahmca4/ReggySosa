...
function renderTournamentDetails(id) {
  ...
  // Bracket — proper visual tree with team slots and connector lines
  if ((tournament.status === 'started' || tournament.status === 'completed') && tournament.bracket) {
    ...
    rounds.forEach((round, rIndex) => {
      ...
      roundLabel.className = 'round-header';
      const totalRounds = rounds.length;
      if (rIndex === totalRounds - 1) {
        roundLabel.textContent = 'Final';
      } else if (rIndex === totalRounds - 2) {
        roundLabel.textContent = 'Semi-Finals';
      } else if (rIndex === totalRounds - 3) {
        roundLabel.textContent = 'Quarter-Finals';
      } else {
        roundLabel.textContent = 'Round ' + (rIndex + 1);
      }
      ...
      round.forEach((match, mIndex) => {
        ...
        matchDiv.className = 'match' + (match.winner ? ' match--has-winner' : '');

        // Team 1 slot
        const team1Slot = document.createElement('div');
        team1Slot.className = 'match-team' +
          (match.winner === match.team1 ? ' match-team--winner' : '') +
          (!match.team1 || match.team1 === 'BYE' || match.team1 === 'TBD' ? ' match-team--bye' : '');
        team1Slot.title = match.team1 || 'TBD';
        const t1Seed = document.createElement('span');
        t1Seed.className = 'match-team-seed';
        t1Seed.textContent = match.winner === match.team1 ? '✓' : '';
        const t1Name = document.createElement('span');
        t1Name.textContent = match.team1 || 'TBD';
        team1Slot.appendChild(t1Seed);
        team1Slot.appendChild(t1Name);
        matchDiv.appendChild(team1Slot);

        // Team 2 slot
        const team2Slot = document.createElement('div');
        team2Slot.className = 'match-team' +
          (match.winner === match.team2 ? ' match-team--winner' : '') +
          (!match.team2 || match.team2 === 'BYE' || match.team2 === 'TBD' ? ' match-team--bye' : '');
        team2Slot.title = match.team2 || 'BYE';
        const t2Seed = document.createElement('span');
        t2Seed.className = 'match-team-seed';
        t2Seed.textContent = match.winner === match.team2 ? '✓' : '';
        const t2Name = document.createElement('span');
        t2Name.textContent = match.team2 || 'BYE';
        team2Slot.appendChild(t2Seed);
        team2Slot.appendChild(t2Name);
        matchDiv.appendChild(team2Slot);

        // Match info bar (code)
        const showCode = role === 'admin' || isUserInMatch(match, tournament);
        const infoBar = document.createElement('div');
        infoBar.className = 'match-info';
        const codeSpan = document.createElement('span');
        codeSpan.className = 'match-code';
        codeSpan.textContent = showCode ? 'Code: ' + match.code : 'Code: hidden';
        infoBar.appendChild(codeSpan);
        matchDiv.appendChild(infoBar);

        // Admin controls
        if (role === 'admin' && (tournament.status === 'started' || tournament.status === 'completed')) {
          const adminControls = document.createElement('div');
          adminControls.className = 'match-admin-controls';

          // Only show reporting if real match (not BYE)
          if (match.team1 && match.team1 !== 'BYE' && match.team2 && match.team2 !== 'BYE' && !match.winner) {
            const reportDiv = document.createElement('div');
            reportDiv.style.cssText = 'display:flex;flex-direction:column;gap:0.4rem;';

            const select = document.createElement('select');
            select.style.cssText = 'width:100%;padding:0.3rem;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text);font-size:0.8rem;';
            const defOpt = document.createElement('option');
            defOpt.value = ''; defOpt.textContent = 'Select winner';
            select.appendChild(defOpt);
            [match.team1, match.team2].forEach(function(tn) {
            ...
            scoreT1.placeholder = 'T1 goals'; scoreT1.className = 'score-input score-admin-input';
            scoreT1.style.cssText = 'flex:1;font-size:0.8rem;padding:0.3rem;';
            const scoreSep = document.createElement('span');
            scoreSep.textContent = '—';
            scoreSep.style.cssText = 'color:var(--text-muted);';
            const scoreT2 = document.createElement('input');
            scoreT2.type = 'number'; scoreT2.min = '0'; scoreT2.max = '99';
            scoreT2.placeholder = 'T2 goals'; scoreT2.className = 'score-input score-admin-input';
            scoreT2.style.cssText = 'flex:1;font-size:0.8rem;padding:0.3rem;';
            ...
            reportBtn.style.cssText = 'font-size:0.78rem;padding:0.35rem 0.6rem;width:100%;';
            reportBtn.addEventListener('click', function() {
              const winnerName = select.value;
              if (!winnerName) { alert('Select a winner.'); return; }
            ...
            adminControls.appendChild(reportDiv);
          }
          ...
          editMatchBtn.style.cssText = 'font-size:0.72rem;padding:0.25rem 0.5rem;background:transparent;border-color:rgba(255,255,255,0.15);color:var(--text-muted);width:100%;';
          editMatchBtn.addEventListener('click', function(e) {
            e.stopPropagation();
            const panelId = 'edit-match-' + rIndex + '-' + mIndex;
            const existing = document.getElementById(panelId);
            if (existing) { existing.remove(); return; }
            document.querySelectorAll('.edit-match-panel').forEach(function(p) { p.remove(); });
            ...
            panel.innerHTML =
              ...
              '<p class="edit-match-note">Pick teams or type custom names. Changing a team resets the winner and generates a new match code.</p>' +
              '<div class="edit-match-row">' +
                '<label>Team 1<select id="em-t1-' + panelId + '">' + allTeamNames.map(function(n) { return '<option value="' + n + '"' + (n === match.team1 ? ' selected' : '') + '>' + n + '</option>'; }).join('') + '</select><input type="text" id="em-t1c-' + panelId + '" placeholder="Or type custom..." style="padding:0.3rem;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text);font-size:0.8rem;width:100%;margin-top:0.3rem;" /></label>' +
                '<span class="edit-match-vs">vs</span>' +
                '<label>Team 2<select id="em-t2-' + panelId + '">' + allTeamNames.map(function(n) { return '<option value="' + n + '"' + (n === match.team2 ? ' selected' : '') + '>' + n + '</option>'; }).join('') + '</select><input type="text" id="em-t2c-' + panelId + '" placeholder="Or type custom..." style="padding:0.3rem;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text);font-size:0.8rem;width:100%;margin-top:0.3rem;" /></label>' +
              '</div>' +
              '<div class="edit-match-actions"><button class="button" id="em-save-' + panelId + '">Save</button><button class="button delete" id="em-cancel-' + panelId + '">Cancel</button></div>' +
              '<p class="edit-match-status" id="em-status-' + panelId + '"></p>';
            matchDiv.appendChild(panel);
            document.getElementById('em-cancel-' + panelId).addEventListener('click', function() { panel.remove(); });
            document.getElementById('em-save-' + panelId).addEventListener('click', async function() {
              ...
              if (newT1 === newT2 && newT1 !== 'BYE') { statusEl.textContent = 'Cannot be the same team.'; return; }
              ...
              try {
                ...
                if (changed) {
                  ...
                  targetMatch.score_team1 = null;
                  targetMatch.score_team2 = null;
                  targetMatch.code = Math.floor(10000 + Math.random() * 90000).toString();
                }
                saveTournaments(tournaments2);
                await fetch(API_BASE_URL + '/api/tournaments/' + tournament.id, {
                  method: 'PUT',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ bracket: bracket2 })
                }).catch(function() {});
                if (supabaseClient) {
                  await supabaseClient.from('tournaments').update({ bracket: bracket2 }).eq('id', tournament.id);
                }
                panel.remove();
                renderTournamentDetails(tournament.id);
              } catch(err) {
          ...
          adminControls.appendChild(editMatchBtn);

          // Advance / Clear buttons
          if (match.winner) {
            const advBtn1 = document.createElement('button');
            advBtn1.className = 'button';
            advBtn1.style.cssText = 'font-size:0.72rem;padding:0.2rem 0.5rem;background:transparent;border-color:rgba(255,255,255,0.15);color:var(--text-muted);width:100%;';
            advBtn1.textContent = '>> Advance ' + match.team1;
            advBtn1.addEventListener('click', function() { advanceTeam(tournament.id, rIndex, mIndex, match.team1); renderTournamentDetails(tournament.id); });
            const advBtn2 = document.createElement('button');
            advBtn2.className = 'button';
            advBtn2.style.cssText = 'font-size:0.72rem;padding:0.2rem 0.5rem;background:transparent;border-color:rgba(255,255,255,0.15);color:var(--text-muted);width:100%;';
            advBtn2.textContent = '>> Advance ' + match.team2;
            advBtn2.addEventListener('click', function() { advanceTeam(tournament.id, rIndex, mIndex, match.team2); renderTournamentDetails(tournament.id); });
            adminControls.appendChild(advBtn1);
            adminControls.appendChild(advBtn2);
          }

          const clearBtn = document.createElement('button');
          clearBtn.className = 'delete';
          clearBtn.style.cssText = 'font-size:0.72rem;padding:0.2rem 0.5rem;width:100%;';
          clearBtn.textContent = '✕ Clear Match';
          clearBtn.addEventListener('click', function() {
            if (!confirm('Clear this match result?')) return;
            clearMatchResult(tournament.id, rIndex, mIndex);
            renderTournamentDetails(tournament.id);
          });
          adminControls.appendChild(clearBtn);
          matchDiv.appendChild(adminControls);
        }

        // Non-admin: report score button if user is in this match
        if (role !== 'admin' && isUserInMatch(match, tournament) && !match.winner) {
          const reportBtn = document.createElement('button');
          reportBtn.className = 'button';
          reportBtn.style.cssText = 'width:100%;font-size:0.8rem;padding:0.4rem;margin-top:0.4rem;';
          reportBtn.textContent = '📸 Report Score';
          reportBtn.addEventListener('click', function() {
            document.getElementById('score-section')?.scrollIntoView({ behavior: 'smooth' });
          });
          const btnWrap = document.createElement('div');
          btnWrap.style.padding = '0.5rem 0.6rem 0.6rem';
          btnWrap.appendChild(reportBtn);
          matchDiv.appendChild(btnWrap);
        }
      ...
      if (rIndex < rounds.length - 1) {
        ...
        connector.style.cssText = 'display:flex;flex-direction:column;width:24px;flex-shrink:0;';
        const nextRoundMatches = rounds[rIndex + 1].length;
        const currentMatches = round.length;
        for (let ci = 0; ci < nextRoundMatches; ci++) {
          const pair = document.createElement('div');
          pair.style.cssText = 'flex:1;display:flex;flex-direction:column;justify-content:space-around;position:relative;';
          // Two lines from current matches converging to one
          const lineTop = document.createElement('div');
          lineTop.style.cssText = 'position:absolute;top:25%;left:0;right:0;height:2px;background:var(--border);';
          const lineBot = document.createElement('div');
          lineBot.style.cssText = 'position:absolute;top:75%;left:0;right:0;height:2px;background:var(--border);';
          const lineMid = document.createElement('div');
          lineMid.style.cssText = 'position:absolute;top:25%;right:0;width:2px;height:50%;background:var(--border);';
          const lineOut = document.createElement('div');
          lineOut.style.cssText = 'position:absolute;top:50%;left:0;right:0;height:2px;background:var(--border);';
          pair.appendChild(lineTop);
          pair.appendChild(lineBot);
          pair.appendChild(lineMid);
          pair.appendChild(lineOut);
...
async function loadTwitchStatus() {
  ...
  // Bracket — full visual bracket with connector lines
  if ((tournament.status === 'started' || tournament.status === 'completed') && tournament.bracket) {
    const bracketHeading = document.createElement('h3');
    bracketHeading.innerHTML = 'Bracket <span class="bracket-live-dot" title="Live — updates automatically"></span>';
    ...
    const numRounds = tournament.bracket.length;
    ...
    tournament.bracket.forEach((round, rIndex) => {
      ...
      roundHeader.textContent = roundNames[rIndex] || ('Round ' + (rIndex + 1));
      ...
      round.forEach((match, mIndex) => {
        ...
        t1.className = 'match-team' +
          (match.winner === match.team1 ? ' match-team--winner' : '') +
          (match.team1 === 'BYE' || match.team1 === 'TBD' ? ' match-team--bye' : '');
        t1.innerHTML = '<span class="match-team-seed"></span><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + (match.team1 || 'TBD') + '</span>';
        matchDiv.appendChild(t1);

        // Team 2 slot
        const t2 = document.createElement('div');
        t2.className = 'match-team' +
          (match.winner === match.team2 ? ' match-team--winner' : '') +
          (match.team2 === 'BYE' || match.team2 === 'TBD' ? ' match-team--bye' : '');
        t2.innerHTML = '<span class="match-team-seed"></span><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + (match.team2 || 'BYE') + '</span>';
        matchDiv.appendChild(t2);

        // Match info row (code + winner)
        const infoDiv = document.createElement('div');
        infoDiv.className = 'match-info';
        const showCode = role === 'admin' || isUserInMatch(match, tournament);
        const codeSpan = document.createElement('span');
        codeSpan.className = 'match-code';
        codeSpan.textContent = showCode ? ('🔑 ' + match.code) : '🔒 Hidden';
        infoDiv.appendChild(codeSpan);
        if (match.winner) {
          const winSpan = document.createElement('span');
          winSpan.style.cssText = 'color:var(--gold);font-weight:700;font-size:0.72rem;';
          winSpan.textContent = '✅ ' + match.winner;
          infoDiv.appendChild(winSpan);
        }
        ...
        if (role === 'admin' && (tournament.status === 'started' || tournament.status === 'completed')) {
          ...
          // Report score (only if both teams real and no winner yet)
          if (match.team1 && match.team1 !== 'BYE' && match.team2 && match.team2 !== 'BYE' && !match.winner) {
            const select = document.createElement('select');
            const defOpt = document.createElement('option');
            defOpt.value = ''; defOpt.textContent = 'Select winner';
            ...
            adminControls.appendChild(select);
            ...
            scoreT1.style.flex = '1';
            const sep = document.createElement('span');
            sep.textContent = '—'; sep.style.cssText = 'color:var(--text-muted);font-size:0.85rem;';
            const scoreT2 = document.createElement('input');
            scoreT2.type = 'number'; scoreT2.min = '0'; scoreT2.max = '99';
            scoreT2.placeholder = match.team2 + ' goals';
            scoreT2.className = 'score-input score-admin-input';
            scoreT2.style.flex = '1';
            scoreRow.appendChild(scoreT1); scoreRow.appendChild(sep); scoreRow.appendChild(scoreT2);
            adminControls.appendChild(scoreRow);
            ...
            adminControls.appendChild(reportBtn);
          }
          ...
          editMatchBtn.className = 'button';
          editMatchBtn.textContent = '✏️ Edit Match';
          editMatchBtn.style.cssText = 'font-size:0.75rem;padding:0.25rem 0.6rem;background:transparent;border-color:rgba(255,255,255,0.2);color:var(--text-muted);';
          editMatchBtn.addEventListener('click', function(e) {
            e.stopPropagation();
            const panelId = 'edit-match-' + rIndex + '-' + mIndex;
            const existing = document.getElementById(panelId);
            ...
            panel.innerHTML =
              '<p class="edit-match-title">✏️ Edit Match Slot</p>' +
              '<p class="edit-match-note">Pick or type a team name. Changing resets winner and generates a new match code.</p>' +
              '<div class="edit-match-row">' +
                '<label>Team 1<select id="em-t1-' + panelId + '">' + allTeamNames.map(function(n){return '<option value="'+n+'"'+(n===match.team1?' selected':'')+'>'+n+'</option>';}).join('') + '</select>' +
                '<input type="text" id="em-t1c-' + panelId + '" placeholder="Or type custom..." style="padding:0.35rem 0.5rem;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text);font-size:0.8rem;width:100%;margin-top:0.25rem;"/></label>' +
                '<span class="edit-match-vs">vs</span>' +
                '<label>Team 2<select id="em-t2-' + panelId + '">' + allTeamNames.map(function(n){return '<option value="'+n+'"'+(n===match.team2?' selected':'')+'>'+n+'</option>';}).join('') + '</select>' +
                '<input type="text" id="em-t2c-' + panelId + '" placeholder="Or type custom..." style="padding:0.35rem 0.5rem;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text);font-size:0.8rem;width:100%;margin-top:0.25rem;"/></label>' +
              '</div>' +
              ...
              '</div>';
            adminControls.appendChild(panel);
            document.getElementById('em-cancel-' + panelId).addEventListener('click', function() { panel.remove(); });
            document.getElementById('em-save-' + panelId).addEventListener('click', function() {
              const t1c = document.getElementById('em-t1c-' + panelId).value.trim();
              const t2c = document.getElementById('em-t2c-' + panelId).value.trim();
              const newT1 = t1c || document.getElementById('em-t1-' + panelId).value;
              const newT2 = t2c || document.getElementById('em-t2-' + panelId).value;
              if (!newT1 || !newT2) { alert('Both team slots must be filled.'); return; }
              const tid = tournament.id;
              const tArr = loadTournaments();
              const tIdx = tArr.findIndex(function(x){return x.id===tid;});
              if (tIdx < 0) return;
              tArr[tIdx].bracket[rIndex][mIndex].team1 = newT1;
              tArr[tIdx].bracket[rIndex][mIndex].team2 = newT2;
              tArr[tIdx].bracket[rIndex][mIndex].winner = null;
              tArr[tIdx].bracket[rIndex][mIndex].code = Math.floor(10000 + Math.random() * 89999).toString();
              saveTournaments(tArr);
              fetch(API_BASE_URL + '/api/tournaments/' + tid, {
                method: 'PATCH',
                headers: {'Content-Type':'application/json'},
                body: JSON.stringify({ bracket: tArr[tIdx].bracket })
              }).catch(function(){});
              panel.remove();
              renderTournamentDetails(tid);
            });
          ...
          advRow.style.cssText = 'display:flex;gap:0.4rem;flex-wrap:wrap;';
          [match.team1, match.team2].forEach(function(tn) {
            if (!tn || tn === 'BYE' || tn === 'TBD') return;
            const advBtn = document.createElement('button');
            advBtn.className = 'button';
            advBtn.style.cssText = 'font-size:0.7rem;padding:0.2rem 0.5rem;background:transparent;border-color:rgba(255,255,255,0.15);color:var(--text-muted);';
            advBtn.textContent = '⏩ Advance ' + tn;
            advBtn.addEventListener('click', function() {
              reportMatchResult(tournament.id, rIndex, mIndex, tn, null, null);
              renderTournamentDetails(tournament.id);
            });
            advRow.appendChild(advBtn);
          });
          adminControls.appendChild(advRow);

          // Clear match button
          const clearBtn = document.createElement('button');
          clearBtn.className = 'button delete';
          clearBtn.style.cssText = 'font-size:0.72rem;padding:0.2rem 0.5rem;';
          clearBtn.textContent = '✖ Clear Match';
          clearBtn.addEventListener('click', function() {
            if (!confirm('Clear this match result?')) return;
            const tid = tournament.id;
            ...
            if (tIdx < 0) return;
            tArr[tIdx].bracket[rIndex][mIndex].winner = null;
            tArr[tIdx].bracket[rIndex][mIndex].score1 = null;
            tArr[tIdx].bracket[rIndex][mIndex].score2 = null;
            saveTournaments(tArr);
            fetch(API_BASE_URL + '/api/tournaments/' + tid, {
              method: 'PATCH',
              headers: {'Content-Type':'application/json'},
        ...
        // Non-admin score submission
        if (role !== 'admin') {
          const userInMatch = isUserInMatch(match, tournament);
          if (userInMatch && !match.winner && match.team1 !== 'BYE' && match.team2 !== 'BYE' && match.team1 !== 'TBD' && match.team2 !== 'TBD') {
              ...
              if (supabaseClient) {
                try {
                  const { data: existingSubs } = await supabaseClient
                    .from('score_submissions').select('id, status')
                    .eq('tournament_id', String(tournament.id))
                    .eq('round_index', rIndex).eq('match_index', mIndex)
                    .in('status', ['pending','approved']).limit(1);
                  if (existingSubs && existingSubs.length > 0) alreadySubmitted = true;
                } catch(e) {}
              }
              if (alreadySubmitted) {
                scoreSlot.innerHTML = '<p style="color:var(--gold);font-size:0.85rem;padding:0.5rem 0.6rem;">✅ Result submitted — waiting for admin confirmation.</p>';
              } else {
      ...
      // Add connector lines between rounds (not after last round)
      if (rIndex < numRounds - 1) {
        const numMatches = round.length;
        const connector = document.createElement('div');
        connector.className = 'round-connector';
        for (let p = 0; p < Math.ceil(numMatches / 2); p++) {
          const pair = document.createElement('div');
          pair.className = 'connector-pair';
          const lineTop = document.createElement('div');
          lineTop.className = 'connector-line top';
          const lineMid = document.createElement('div');
          lineMid.className = 'connector-line mid';
          const lineBot = document.createElement('div');
          lineBot.className = 'connector-line bottom';
          const lineOut = document.createElement('div');
          lineOut.className = 'connector-line out';
          pair.appendChild(lineTop);
          pair.appendChild(lineMid);
          pair.appendChild(lineBot);
          pair.appendChild(lineOut);
    ...
    detail.appendChild(bracketHeading);
    detail.appendChild(bracketWrapper);lements user authentication, tournament management and
 * bracket generation entirely in the browser using localStorage.
 *
 * NOTE: This implementation is for demo purposes only and does not
 * persist data across different browsers or users. For a production
